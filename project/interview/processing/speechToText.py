...
def speechProcessing(src_filename, dst_filename):
    ...
    subprocess.call("gsutil -m mv -r testaudio.flac gs://capdi_test/testupload2.flac",shell=True)
    ...
    # Speech to text 요청
    result = json.loads(subprocess.check_output("gcloud ml speech recognize-long-running gs://capdi_test/testaudio.flac --language-code=ko-KR --encoding=flac --sample-rate=48000 --async", shell=True))
